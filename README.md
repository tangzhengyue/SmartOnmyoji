...
1. 选择使用的客户端，推荐使用 **阴阳师桌面版**（[NGA论坛](https://g.nga.cn/read.php?tid=29661629)）、桌面版多开器推荐（[V5多开器](https://soft.3dmgame.com/down/167157.html)）、MuMu模拟器([官网](https://yys.163.com/zmb/))
2. 选择单开模式还是多开模式，初次使用建议单开
3. 点击选择窗体，在倒计时结束前点击窗体，结束后显示窗口标题
4. 选择目标名称，初次使用推荐选择 **[御魂]** 模式，在软件目录下打开 **./img/yuhun/** ,文件夹下的图片就是御魂模式需要匹配的目标
5. 调整脚本运行时间，默认即可，最好不超过三小时
6. 调整匹配间隔，默认3秒即可
7. 调整坐标偏移，可以在安全性和准确性之间权衡，默认即可
8. 选择运行模式，推荐正常模式
   + 正常模式：即后台模式(不能点击最小化，可以被其他窗口覆盖，可以正常使用鼠标)，但不支持部分窗体（如网易MuMu、网易云游戏）
   + 兼容模式：即前台模式(始终保持窗口最前，不能正常使用鼠标)，支持所有窗体
9. 调整压缩截图，设置在0.6左右时，准确率和速度能兼顾，如果对匹配速度可以忍受，不建议修改压缩率（压缩率越低，准确度越低）
10. 设置匹配方法，初次使用推荐选择特征点匹配
    + 模板匹配, 速度快，对目标图片要求一模一样
    + 特征点匹配，速度慢，对目标图片要求看起来差不多就行了
11. 选择脚本结束后操作，默认即可，字面意思
12. 若你使用桌面版客户端，请勾选**桌面版防异常**，请打开配置文件**config.ini**修改window系统屏幕缩放率，即**[other_setting]** -> **srceen_scale_rate**
13. 最后点击开始，即可运行并且输出运行日志
...
- 2022年11月02日 优化：完善使用说明，新增：配置文件增加关闭记录日志的参数
...
- 2022年05月09日 修复部分异常情况的提示音（不喜欢可以通过配置文件关闭）
...
- 2022年01月19日 优化匹配速度，截图和读取模板图片时先转灰度，支持对截图设置压缩率，进一步优化匹配速度；更换全部匹配方法为sift匹配
